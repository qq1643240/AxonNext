...
void updateViewConfiguration() {
    if (initialized && [AXNManager sharedInstance].view) {
        ...
        [AXNManager sharedInstance].view.animationStyle = animationStyle;
    }
}
...
%ctor {
  NSLog(@"[Axon] init");
  CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, (CFNotificationCallback)loadPrefs, CFSTR("me.nepeta.axonnext/ReloadPrefs"), NULL, CFNotificationSuspensionBehaviorCoalesce);
  loadPrefs();
