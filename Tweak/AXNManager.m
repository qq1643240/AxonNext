...
+(instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharedInstance.mutedApps = [NSMutableDictionary new];
        sharedInstance.favoritedApps = [NSMutableDictionary new];
        sharedInstance.whitelist = [NSMutableArray new];
        sharedInstance.blacklist = [NSMutableArray new];
...
-(id)init {
  [[objc_getClass("NSDistributedNotificationCenter") defaultCenter] addObserver:self selector:@selector(clearAll) name:@"me.nepeta.axonnext.clearAllNotification" object:nil];
  [[objc_getClass("NSDistributedNotificationCenter") defaultCenter] addObserver:self selector:@selector(saveNotificationForDebug) name:@"me.nepeta.axonnext.saveNotification" object:nil];
  [self loadFilters];
  return self;
}

-(void)loadFilters {
    NSDictionary *prefs = [[NSMutableDictionary alloc] initWithContentsOfFile:@"/var/mobile/Library/Preferences/me.nepeta.axon.plist"];
    self.whitelist = [NSMutableArray arrayWithArray:prefs[@"Whitelist"] ?: @[]];
    self.blacklist = [NSMutableArray arrayWithArray:prefs[@"Blacklist"] ?: @[]];
    self.mutedApps = [NSMutableDictionary dictionaryWithDictionary:prefs[@"MutedApps"] ?: @{}];
    self.favoritedApps = [NSMutableDictionary dictionaryWithDictionary:prefs[@"FavoritedApps"] ?: @{}];
}

-(BOOL)isBundleAllowed:(NSString *)bundleIdentifier {
    // Blacklist takes priority
    if ([self.blacklist containsObject:bundleIdentifier]) return NO;
    // If whitelist is non-empty, only allow whitelisted
    if ([self.whitelist count] > 0 && ![self.whitelist containsObject:bundleIdentifier]) return NO;
    return YES;
}

-(BOOL)isMuted:(NSString *)bundleIdentifier {
    return [self.mutedApps[bundleIdentifier] boolValue];
}

-(void)setMuted:(BOOL)muted forBundleIdentifier:(NSString *)bundleIdentifier {
    self.mutedApps[bundleIdentifier] = @(muted);
    [self saveFilters];
}

-(void)toggleMuteForBundleIdentifier:(NSString *)bundleIdentifier {
    BOOL current = [self isMuted:bundleIdentifier];
    [self setMuted:!current forBundleIdentifier:bundleIdentifier];
}

-(BOOL)isFavorited:(NSString *)bundleIdentifier {
    return [self.favoritedApps[bundleIdentifier] boolValue];
}

-(void)toggleFavoriteForBundleIdentifier:(NSString *)bundleIdentifier {
    BOOL current = [self isFavorited:bundleIdentifier];
    self.favoritedApps[bundleIdentifier] = @(!current);
    [self saveFilters];
}

-(void)saveFilters {
    NSMutableDictionary *prefs = [[[NSMutableDictionary alloc] initWithContentsOfFile:@"/var/mobile/Library/Preferences/me.nepeta.axon.plist"] mutableCopy] ?: [NSMutableDictionary new];
    prefs[@"Whitelist"] = [self.whitelist copy];
    prefs[@"Blacklist"] = [self.blacklist copy];
    prefs[@"MutedApps"] = [self.mutedApps copy];
    prefs[@"FavoritedApps"] = [self.favoritedApps copy];
    [prefs writeToFile:@"/var/mobile/Library/Preferences/me.nepeta.axon.plist" atomically:YES];
}
...
-(NSString *)notificationContentForBundleIdentifier:(NSString *)bundleIdentifier {
    ...
    NSString *appName = self.names[bundleIdentifier] ?: bundleIdentifier;
    [content appendFormat:@"%@ (%ld 条通知)\n\n", appName, (long)[requests count]];
    
    for (int i = 0; i < [requests count] && i < 10; i++) {
        NCNotificationRequest *req = requests[i];
        NSString *title = req.content.title ?: @"";
        NSString *body = req.content.body ?: @"";
...
-(void)saveNotificationForDebug {
  ...
  [[array description] writeToFile:@"/var/mobile/Documents/AxonNextDebug.txt" atomically:false encoding:NSUTF8StringEncoding error:nil];
}
...
-(UIImage *)getIcon:(NSString *)bundleIdentifier {
    ...
    if (!image) {
      NSLog(@"[AxonNext] Image Not Found!");
        NSArray *requests = [self requestsForBundleIdentifier:bundleIdentifier];
...
-(void)insertNotificationRequest:(NCNotificationRequest *)req {
    ...
    // Filter check
    if (![self isBundleAllowed:bundleIdentifier]) return;
