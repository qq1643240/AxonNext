...
-(void)toggleMuteForBundleIdentifier:(NSString *)bundleIdentifier {
    ...
    static NSMutableSet *mutedApps = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        mutedApps = [NSMutableSet new];
    });

    if ([mutedApps containsObject:bundleIdentifier]) {
        [mutedApps removeObject:bundleIdentifier];
    } else {
        [mutedApps addObject:bundleIdentifier];
    }
}

-(void)pinBundleIdentifier:(NSString *)bundleIdentifier {
    if (!bundleIdentifier) return;

    static NSMutableDictionary *pinnedApps = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        pinnedApps = [NSMutableDictionary new];
    });

    pinnedApps[bundleIdentifier] = @([[NSDate date] timeIntervalSince1970]);
}

-(void)toggleFavoriteForBundleIdentifier:(NSString *)bundleIdentifier {
    if (!bundleIdentifier) return;

    static NSMutableSet *favoriteApps = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        favoriteApps = [NSMutableSet new];
    });

    if ([favoriteApps containsObject:bundleIdentifier]) {
        [favoriteApps removeObject:bundleIdentifier];
    } else {
        [favoriteApps addObject:bundleIdentifier];
    }
}
