...
-(id)initWithFrame:(CGRect)frame {
    ...
    NSString *prefsPath = @"/var/jb/Library/Preferences/me.nepeta.axonnext.plist";
    if (![[NSFileManager defaultManager] fileExistsAtPath:prefsPath]) {
        prefsPath = @"/var/mobile/Library/Preferences/me.nepeta.axonnext.plist";
    }
...
-(void)axnViewNotifications {
    if (self.bundleIdentifier) {
        [[AXNManager sharedInstance] showNotificationRequestsForBundleIdentifier:self.bundleIdentifier];
    }
}
-(void)axnMuteNotifications {
    if (self.bundleIdentifier) {
        [[AXNManager sharedInstance] toggleMuteForBundleIdentifier:self.bundleIdentifier];
    }
}
-(void)axnCopyNotificationContent {
    if (self.bundleIdentifier) {
...
-(void)axnFavorite {
    if (self.bundleIdentifier) {
        [[AXNManager sharedInstance] toggleFavoriteForBundleIdentifier:self.bundleIdentifier];
    }
}
...
-(BOOL)canPerformAction:(SEL)action withSender:(id)sender {
    return (action == @selector(axnClearAll) ||
            action == @selector(axnRealClearAll) ||
            action == @selector(axnOpenApp) ||
            action == @selector(axnViewNotifications) ||
            action == @selector(axnMuteNotifications) ||
            action == @selector(axnCopyNotificationContent) ||
            action == @selector(axnFavorite));
}
