...
- (void)performSwipeAction:(NSInteger)action {
    switch (action) {
        ...
        case 5:
            ...
            break;
        case 6:
            [self muteSelectedAppNotifications];
            break;
        case 7:
            [self pinSelectedApp];
            break;
...
- (void)muteSelectedAppNotifications {
    if (self.hapticFeedback) AudioServicesPlaySystemSound(1519);
    if (self.selectedBundleIdentifier) {
        [[AXNManager sharedInstance] toggleMuteForBundleIdentifier:self.selectedBundleIdentifier];
    }
}

- (void)pinSelectedApp {
    if (self.hapticFeedback) AudioServicesPlaySystemSound(1521);
    if (self.selectedBundleIdentifier) {
        [[AXNManager sharedInstance] pinBundleIdentifier:self.selectedBundleIdentifier];
    }
}
