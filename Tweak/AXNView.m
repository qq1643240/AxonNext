...
// Animation helper
static void axn_animateWithStyle(NSInteger style, void (^animations)(void)) {
    switch (style) {
        case 0: // Apple Spring
            [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:0.7 initialSpringVelocity:0.5 options:UIViewAnimationOptionCurveEaseInOut animations:animations completion:NULL];
            break;
        case 1: // Scale
            [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:animations completion:NULL];
            break;
        case 2: // Bounce
            [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.3 initialSpringVelocity:0.8 options:UIViewAnimationOptionCurveEaseInOut animations:animations completion:NULL];
            break;
        case 3: // Fluid
            [UIView animateWithDuration:0.6 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:0.3 options:UIViewAnimationOptionCurveEaseInOut animations:animations completion:NULL];
            break;
        case 4: // Jelly
            [UIView animateWithDuration:0.7 delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:0.9 options:UIViewAnimationOptionCurveEaseInOut animations:animations completion:NULL];
            break;
        case 5: // Fade
            [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveEaseIn | UIViewAnimationOptionTransitionCrossDissolve animations:animations completion:NULL];
            break;
        default:
...
-(id)initWithFrame:(CGRect)frame {
    ...
    self.swipeUpAction = 1;     // open app
    self.swipeDownAction = 2;   // clear notifications
    self.swipeLeftAction = 3;   // mute
    self.swipeRightAction = 4;  // favorite/pin
...
- (void)setAnimationStyle:(NSInteger)animationStyle {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.gesturesEnabled = self.gesturesEnabled;
    cell.swipeUpAction = self.swipeUpAction;
    cell.swipeDownAction = self.swipeDownAction;
    cell.swipeLeftAction = self.swipeLeftAction;
    cell.swipeRightAction = self.swipeRightAction;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    CGFloat size = self.iconSize ?: 60;
    if (self.badgesEnabled) {
        switch (self.style) {
            case 1: return CGSizeMake(size, size);
            case 2: return CGSizeMake(size * 0.75, size * 0.75);
            case 3: return CGSizeMake(size * 0.625, size);
            case 4: return CGSizeMake(size, size * 0.5);
            case 5: return CGSizeMake(size, size * 0.6);
            default: return CGSizeMake(size, size + 26);
        }
    } else if (!self.badgesEnabled) {
        switch (self.style) {
            case 1: return CGSizeMake(size, size);
            case 2: return CGSizeMake(size * 0.75, size * 0.75);
            case 3: return CGSizeMake(size * 0.625, size * 0.625);
            case 4: return CGSizeMake(size, size * 0.5);
            case 5: return CGSizeMake(size, size * 0.6);
            default: return CGSizeMake(size, size);
        }
    }
...
- (UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section {
    ...
    CGFloat width = self.iconSize ?: 60;
    CGFloat viewWidth = self.bounds.size.width;

    if (self.style == 2) width = width * 0.75;
    else if (self.style == 3) width = width * 0.625;
...
- (void)refresh {
    ...
    if (self.autoArrange) {
        ...
        for (NSString *key in sortedKeys) {
            if ([[AXNManager sharedInstance] isFavorited:key]) {
                [favorited addObject:key];
            } else {
                [normal addObject:key];
            }
        }
        sortedKeys = [favorited arrayByAddingObjectsFromArray:normal];
    }
